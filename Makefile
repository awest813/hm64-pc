...
recomp-generate: $(BASENAME).elf $(N64RECOMP_TOOL)
	...
	@grep -q "osPfsNumFiles_recomp" $(RECOMP_OUTPUT_DIR)/funcs/funcs.h || \
		sed -i 's/void osPfsFindFile_recomp(uint8_t\* rdram, recomp_context\* ctx);/void osPfsFindFile_recomp(uint8_t* rdram, recomp_context* ctx);\nvoid osPfsNumFiles_recomp(uint8_t* rdram, recomp_context* ctx);\nvoid osPfsRepairId_recomp(uint8_t* rdram, recomp_context* ctx);/' \
		$(RECOMP_OUTPUT_DIR)/funcs/funcs.h
