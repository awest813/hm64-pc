...
namespace RT64 {
    ...
    GBI *GBIManager::getGBIForUCode(uint8_t *RDRAM, uint32_t textAddress, uint32_t dataAddress) {
        ...
        if (textSegmentIndex < 0 || dataSegmentIndex < 0) {
            ...
            if ((textAddress == 0x000EFA80) && (dataAddress == 0x000FC7C0)) {
                fprintf(stderr, "[hm64] Falling back to F3DEX2.fifo 2.05 for HM64 ucode addresses.\n");
                GBI &gbi = gbiCache[uint32_t(F3DEX2_FIFO_2_05.ucode)];
                if (gbi.ucode == GBIUCode::Unknown) {
                    gbi.ucode = F3DEX2_FIFO_2_05.ucode;
                    GBI_RDP::setup(&gbi, true);
                    GBI_F3DEX2::setup(&gbi);
                }

                gbi.flags = F3DEX2_FIFO_2_05.flags;
                return &gbi;
            }
