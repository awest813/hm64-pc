...
#       endif
    ...
    void RSP::modifyVertex(uint16_t dstIndex, uint16_t dstAttribute, uint32_t value) {
        ...
        if ((globalIndex * 4 + 3 >= normColBytes.size()) ||
            (globalIndex * 2 + 1 >= tcFloats.size()) ||
            (globalIndex >= tcVelFloats.size() / 2) ||
            (globalIndex >= fogIndices.size()) ||
            (globalIndex >= lightIndices.size()) ||
            (globalIndex >= lightCounts.size()) ||
            (globalIndex >= lookAtIndices.size()) ||
            (globalIndex >= posScreen.size())) {
            return;
        }
        ...
        if (used[dstIndex]) {
            ...
            auto &worldIndices = workload.drawData.worldIndices;
            auto &posTransformed = workload.drawData.posTransformed;
            if ((globalIndex * 3 + 2 >= posFloats.size()) ||
                (globalIndex * 3 + 2 >= velFloats.size()) ||
                (globalIndex >= viewProjIndices.size()) ||
                (globalIndex >= worldIndices.size()) ||
                (globalIndex >= posTransformed.size())) {
                return;
            }
    ...
    void RSP::branchZ(uint32_t branchDl, uint16_t vtxIndex, uint32_t zValue, DisplayList **dl) {
        ...
        if ((branchDl != 0) && (forceBranch || (screenZ < zValueFloat))) {
    ...
    void RSP::branchW(uint32_t branchDl, uint16_t vtxIndex, uint32_t wValue, DisplayList **dl) {
        ...
        if ((branchDl != 0) && (forceBranch || (posW < static_cast<float>(wValue)))) {
            const uint32_t rdramAddress = fromSegmentedMasked(branchDl);
            *dl = reinterpret_cast<DisplayList *>(state->fromRDRAM(rdramAddress)) - 1;
        }
    }
    ...
    void RSP::drawIndexedTri(uint32_t a, uint32_t b, uint32_t c, bool rawGlobalIndices) {
        ...
        if (rawGlobalIndices) {
            ...
            if ((a >= indices.size()) || (b >= indices.size()) || (c >= indices.size())) {
                return;
            }
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if ((globalIndex >= worldIndices.size()) ||
                (globalIndex >= posScreen.size()) ||
                (globalIndex * 2 + 1 >= tcFloats.size())) {
                return;
            }
